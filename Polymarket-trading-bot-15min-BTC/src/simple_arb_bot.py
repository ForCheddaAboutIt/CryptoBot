...
# Inside SimpleArbitrageBot class
    ...
    def __init__(self, settings):
        ...
        # --- CHANGE START ---
        # Just initialize placeholders here. DO NOT fetch data yet.
        self.market_slug = None
        self.market_id = None
        self.yes_token_id = None
        self.no_token_id = None
        self.market_end_timestamp = None
        self.market_start_timestamp = None
        # --- CHANGE END ---
    ...
    async def initialize(self):
        """
        Async initialization to find market and load details.
        Call this immediately after creating the bot instance.
        """
        try:
            # 1. Find market (async)
            if self.settings.market_slug:
                logger.info(f"Using configured market: {self.settings.market_slug}")
                self.market_slug = self.settings.market_slug
            else:
                # properly await the async finder
                self.market_slug = await find_current_btc_15min_market_async()
            
            # 2. Fetch Market Details
            logger.info(f"Getting market information: {self.market_slug}")
            # fetch_market_from_slug is synchronous, so we call it normally
            ...
            # 15min markets close 900s after start
        ...
        try:
            while True:
                ...
                if self.get_time_remaining() == "CLOSED":
                    ...
                    try:
                        new_market_slug = await find_current_btc_15min_market_async()
